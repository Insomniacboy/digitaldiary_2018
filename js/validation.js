...
(function () {
  ...
  $('form').on('submit', function (e) {      // При отправке формы
    ...
    // пароль (здесь вы можете кэшировать имя в переменной)
    if (!validateName()) {          // Вызываем validateName(), и если ввод ошибочен,
      showErrorMessage(document.getElementById('name')); // отображаем сообщение об ошибке
      valid.password = false;           // Обновляем объект valid - данный элемент ошибочен
    } else {                            // В противном случае удаляем сообщение об ошибке
  ...
  function validateName() {
    var name = document.getElementById('name'); // Сохраняем ссылку на элемент
    var valid = name.value.length >= 3;				// Если его значение >= 3 символам
    if (!valid) {										// Если нет, устанавливаем сообщение об ошибке
      setErrorMessage(name, 'Логин должно состоять из не менее 3 символов');
    }
    return valid;										// Возвращаем true / false
  }
